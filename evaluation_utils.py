...
def _compute_class_metrics(confusion_matrix: np.ndarray, num_classes: int):
    """Compute per-class precision, recall, and F1 from a confusion matrix.

    Args:
        confusion_matrix: Square confusion matrix of shape (num_classes, num_classes)
        num_classes: Number of classes

    Returns:
        Tuple of (precision_per_class, recall_per_class, f1_per_class) lists
    """
    precision_per_class = []
    recall_per_class = []
    f1_per_class = []
...
def _compute_weighted_averages(
    ...
    """Compute weighted averages of precision, recall, and F1."""
    total_support = sum(support_per_class)
    weighted_precision = (
        sum(p * s for p, s in zip(precision_per_class, support_per_class)) / total_support
    )
    weighted_recall = (
        sum(r * s for r, s in zip(recall_per_class, support_per_class)) / total_support
    )
    weighted_f1 = (
...
def calculate_metrics_manual(
    ...
    precision_per_class, recall_per_class, f1_per_class = _compute_class_metrics(
        confusion_matrix, num_classes
    )

    support_per_class = [confusion_matrix[i, :].sum() for i in range(num_classes)]
    weighted_precision, weighted_recall, weighted_f1 = _compute_weighted_averages(
        precision_per_class, recall_per_class, f1_per_class, support_per_class
    )
