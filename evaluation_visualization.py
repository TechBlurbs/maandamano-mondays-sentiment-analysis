...
def create_text_bar_chart(data, title="Bar Chart", max_width=50):
    ...
    for label, value in data.items():
        ...
        # Format the display
        print(f"{label:<15} |{bar_str:<{max_width}} {value:.2f}")
    print()
...
def _print_grade_assessment(accuracy, macro_f1, threshold_good, threshold_excellent):
    """Print overall assessment grade."""
    print("\nOverall Assessment:")
...
def _print_recommendations(metrics, accuracy, macro_f1, threshold_good):
...
def create_performance_summary(metrics, threshold_good=0.7, threshold_excellent=0.85):
    ...
    # Overall assessment
    _print_grade_assessment(accuracy, macro_f1, threshold_good, threshold_excellent)

    # Detailed analysis
    print("\nDetailed Analysis:")
    acc_label = (
        'Excellent' if accuracy >= threshold_excellent
        else 'Good' if accuracy >= threshold_good
        else 'Needs Improvement'
    )
    f1_label = (
        'Excellent' if macro_f1 >= threshold_excellent
        else 'Good' if macro_f1 >= threshold_good
        else 'Needs Improvement'
    )
    print(f"• Accuracy: {accuracy:.4f} ({acc_label})")
    print(f"• Macro F1: {macro_f1:.4f} ({f1_label})")
    ...
    if 'f1_per_class' in metrics:
        ...
        balance_label = (
            'Balanced' if f1_range < 0.15
            else 'Moderate Imbalance' if f1_range < 0.3
            else 'Significant Imbalance'
        )
        print(f"• Class Balance: {f1_range:.4f} F1 range ({balance_label})")

    # Recommendations
    _print_recommendations(metrics, accuracy, macro_f1, threshold_good)
