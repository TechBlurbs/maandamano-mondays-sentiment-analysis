...
def analyze_user_data(data):
    ...
    multi_tweet_users = len(
        [user for user, count in user_tweet_counts.items() if count > 1]
    )
    most_active_user = (
        user_tweet_counts.most_common(1)[0][0] if user_tweet_counts else "None"
    )
    avg_tweets_per_user = (
        sum(user_tweet_counts.values()) / len(user_tweet_counts)
        if user_tweet_counts
        else 0
    )
...
def main():
    ...
    # Save report
    report_filename = (
        f"maandamano_sentiment_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.html"
    )
